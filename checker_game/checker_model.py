...
class CheckerModel:
	...
	def ia_move(self, model):
		if model == "random" :
		# The code snippet `selected_piece_position, move_position = self.random_model_predict()
		# self.move_piece(selected_piece_position, move_position)` is selecting a random piece position and
		# a random move for that piece from the possible moves generated by the `random_model_predict`
		# method.
